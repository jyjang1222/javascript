...
1. 위 코드 조각은 자바스크립트에게 이렇게 지시한다. "이 버튼이 클릭되면 add를 실행해".
...
4. 객체의 요소에서 왼쪽은 키(key) 또는 속성으로 불리며 오른쪽은 값(value)으로 불린다.
...
- defer
  ...
  - **defer** 는 스크립트를 바로 다운로드 하지만 html 구문분석도 정지가 안되게 해서 구문분석이 계속 이루어지게 해준다. 그리하여 스크립트를 미리 다운로드 하고 html 구문분석이 완료되면 스크립트가 실행되도록 해준다.
- async
