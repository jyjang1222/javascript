...
- 오류가 발생할지 모르는 코드를 제어하고 오류에 대응한다.
...
- catch 내부의 코드에서는 catch에 의해 잡힌 오류를 사용할 수 있다.
- catch 내부의 코드에서 오류발생시의 처리로직을 작성할 수 있다.

### finally 키워드
```javascript
//try catch키워드의 예시 코드에서 이어짐..
let inputNumber;
...
```
- finally 키워드는 오류가 있든 없든 finally내부의 코드를 실행한다.
- finally 키워드는 catch에서 발생한 오류 이외에 발생할수있는 추가적인 오류를 디버깅하는데 쓰인다.
- catch코드 내부에 오류를 다시 발생시키고 자체 분석 서버에 오류를 기록한다.
- 오류를 다시 발생시키면 스크립트는 종료되고 finally 이후의 코드는 실행되지 않게 된다.
- 특히 네트워크 관련 요청 작업을 할때 쓰인다고 한다.
